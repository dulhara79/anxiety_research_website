...
export default function TextReveal({
  ...
  return <MotionTag
    ...
    initial={reduced ? false : { opacity: 0, x: hiddenX, y: hiddenY, filter: `blur(${settings.blur}px)` }}
    whileInView={reduced ? undefined : { opacity: 1, x: 0, y: 0, filter: 'blur(0px)' }}
    viewport={{ once: false, amount: amount ?? settings.amount, margin: '-8% 0px -10% 0px' }}
    transition={{ duration: settings.duration, delay, ease: [0.22, 1, 0.36, 1] }}
  >
    {children}
  </MotionTag>
}
