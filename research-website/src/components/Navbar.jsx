...
export default function Navbar() {
  ...
  const spring = reduced ? { duration: 0 } : { type: 'spring', stiffness: 280, damping: 30, mass: 0.82 }
  ...
  return <header className={`site-header ${compact ? 'site-header--compact' : ''}`}>
    ...
    >
      <motion.button
        ...
        animate={{ opacity: compact ? 0 : 1, width: compact ? 0 : 'auto', marginRight: compact ? 0 : 18 }}
        transition={spring}
      ...
      <motion.nav aria-label="Primary" className={open ? 'nav-links open' : 'nav-links'} layout="position" transition={{ layout: spring }}>
      ...
      <motion.button
        ...
        animate={{ opacity: compact ? 0 : 1, width: compact ? 0 : 'auto', paddingLeft: compact ? 0 : 16, paddingRight: compact ? 0 : 16, marginLeft: compact ? 0 : 18 }}
        transition={spring}
        whileHover={compact ? undefined : { y: -2, scale: 1.02 }}
        whileTap={compact ? undefined : { scale: 0.98 }}
        tabIndex={compact ? -1 : 0}
        aria-hidden={compact}
      >Explore Research</motion.button>
    </motion.div>
  </header>
}
